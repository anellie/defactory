...
class PhysicsEngine {
    ...
    private static final float PHYSICS_ENTITY_SIZE = ENTITY_SIZE / 2.075f;
    ...
    PhysicsEngine(Player player) {
        ...
        playerDef.position.set(player.getPosition()).add(ENTITY_SIZE / 2f, ENTITY_SIZE / 2f);
    ...
    void act(float deltaTime) {
        ...
        player.getPosition().set(playerBody.getPosition().sub(ENTITY_SIZE / 2, ENTITY_SIZE / 2));
    }
    ...
    void blockPlaced(TileVector position) {
        blockDef.position.set(position.getX(), position.getY()).add(0.5f, 0.5f);
